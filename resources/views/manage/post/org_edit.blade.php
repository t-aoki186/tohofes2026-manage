<x-manage-layout>
    ...
    <form action="{{ route('manage.post.organization.store') }}" method="POST" class="flex flex-col gap-4 edit-form">
        @csrf
        {{-- 編集時は元のIDを保持、新規時は空 --}}
            ...
            <details class="accordion-main mb-4 min-w-full border border-[#2c2d30] p-4 rounded-xl">
                ...
                <div class="mb-4 grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="text" name="orgDate" value="{{ $target['orgDate'] ?? '' }}" class="w-full bg-[#2c2d30] text-white rounded-xl p-3" placeholder="例: 1, 2">
                    </div>
                    <div class="flex flex-col gap-1">
                ...
                <div class="mb-6">
                    ...
                    <div id="schedule-container" class="space-y-3">
                        ...
                        <div class="flex gap-2 schedule-row">
                            <input type="text" name="schedule[{{ $index }}][day]" value="{{ $sch['day'] }}" placeholder="日" class="w-16 bg-[#2c2d30] text-white rounded-lg p-2 text-center">
                            <input type="text" name="schedule[{{ $index }}][time]" value="{{ $sch['time'] }}" placeholder="10:30 ~ 11:30" class="flex-1 bg-[#2c2d30] text-white rounded-lg p-2">
        ...
        <div class="mt-6 flex justify-between items-center">
            <a href="{{ route('manage.post.organization.index') }}" class="follow-btn">← 一覧に戻る</a>
